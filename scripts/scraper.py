    ...
    def query_overpass(self, query: str, category: str) -> List[POI]:
        ...
        try:
            ...
            for element in data.get('elements', []):
                ...
                # Skip POIs without a proper name
                if not name:
                    skipped_unnamed += 1
                    continue
            ...
            if skipped_unnamed:
                print(f"  Found {len(pois)} {category} (skipped {skipped_unnamed} unnamed)")
            else:
            ...
            try:
                ...
                for row in reader:
                    try:
                        ...
                        # DoBIH CSVs only have Grid Ref, not direct lat/lng
                        ...
                        # Get elevation
                        height_str = row.get('Height (m)', '0')
                        elevation = float(str(height_str).replace('m', '').strip())

                        # Get region/area
                        region = row.get('SMC Section', row.get('RHB Section', 'Unknown'))

                        # Get map references
                        map_50k = row.get('Map 1:50k', '')
                        map_25k = row.get('Map 1:25k', '')
                        ...
                        # Add DoBIH number if available
                        dobih_num = row.get('DoBIH Number', '')
                        if dobih_num:
                            attributes['dobih_number'] = dobih_num
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Scrape Scotland POI data from various sources')
    parser.add_argument('--lite', action='store_true',
                        help='Quick test mode (limited categories, no images)')
    parser.add_argument('--categories', type=str,
                        help='Comma-separated list of categories to scrape')
    parser.add_argument('--limit', type=int, default=0,
                        help='Max POIs per category (0 = unlimited)')
    ...
    parser.add_argument('--zapmap-key', type=str,
                        help='Zap-Map API key for EV charger data')

    args = parser.parse_args()

    # Available categories
    ALL_CATEGORIES = ['campsites', 'parking', 'mountains', 'lochs', 'beaches',
                      'fuel', 'ev_charging', 'historic', 'viewpoints', 'walks']
    LITE_CATEGORIES = ['campsites', 'beaches', 'mountains']
    ...
    if args.lite:
        ...
        print("🚀 Running in LITE mode (limited categories, no images)")
    ...
    for category in categories:
        if category == 'walks':
            # Hiking routes use a special method
            pois = scraper.fetch_hiking_routes()
        elif category in scraper.OSM_QUERIES:
            pois = scraper.query_overpass(scraper.OSM_QUERIES[category], category)
        else:
            print(f"  Unknown category: {category}, skipping...")
            continue

        # Apply limit if specified
        if limit > 0 and len(pois) > limit:
            pois = pois[:limit]
            print(f"  Limited to {limit} POIs")

        scraper.all_pois.extend(pois)
        time.sleep(2)

    # DoBIH hills (if mountains category selected)
    if 'mountains' in categories:
        print("\n=== Fetching DoBIH hills ===")
        dobih_pois = scraper.fetch_dobih_hills()
        if limit > 0 and len(dobih_pois) > limit:
            dobih_pois = dobih_pois[:limit]
        scraper.all_pois.extend(dobih_pois)

    # Zap-Map (if ev_charging category selected and API key provided)
    if 'ev_charging' in categories and args.zapmap_key:
        print("\n=== Fetching Zap-Map EV chargers ===")
        zapmap_pois = scraper.fetch_zapmap_chargers()
        if limit > 0 and len(zapmap_pois) > limit:
            zapmap_pois = zapmap_pois[:limit]
        scraper.all_pois.extend(zapmap_pois)

    # Image enrichment
    if not skip_images:
        scraper.enrich_with_images()
    else:
        print("\n⏭️  Skipping image enrichment")
