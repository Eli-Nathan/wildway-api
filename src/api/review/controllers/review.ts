...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
      ...
      ctx.body = {
        data: reviews.map((r: any) => ({
          ...
          author: {
            ...
            avatar: r.owner?.profile_pic?.url || r.owner?.avatar || null,
          },
          image: r.image?.url ? { url: r.image.url } : null,
     ...
     */
    async addReply(ctx: StrapiContext) {
     ...
     * The policy stores the review in ctx.state.review to avoid duplicate queries.
     */
    async deleteReply(ctx: StrapiContext) {
      const { id: reviewId } = ctx.params;
